...
/usr/local/bin/aws s3 cp  s3://${s3_bucket}/${dsf_install_tarball_path} .
...
/usr/local/bin/aws secretsmanager get-secret-value --secret-id ${federation_private_key_name} --query SecretString --output text > /home/sonarw/.ssh/id_rsa
/usr/local/bin/aws secretsmanager get-secret-value --secret-id ${federation_public_key_name} --query SecretString --output text > /home/sonarw/.ssh/id_rsa.pub
/usr/local/bin/aws secretsmanager get-secret-value --secret-id ${federation_public_key_name} --query SecretString --output text > /home/sonarw/.ssh/authorized_keys
