...
sed '3 a export ITP_HOME=/opt/itp\nITPBA_HOME=/opt/itpba\nexport CATALINA_HOME=/opt/apache-tomcat' /opt/itp_global_conf/auto_deploy.sh > /opt/itp_global_conf/auto_deploy_tf.sh
chmod +x /opt/itp_global_conf/auto_deploy_tf.sh
while ! nc -z ${admin_server_ip} 8443; do
  sleep 0.1
done
sudo /opt/itp_global_conf/auto_deploy_tf.sh --hostname "$(hostname)" --ip-address "$my_ip" --dns-servers "$my_nameserver" --registration-password "${registration_password}" --cidr "$my_cidr" --default-gateway "$my_default_gw" --machine-type "Analytics" --analytics-user "${analytics_user}" --analytics-password "${analytics_password}" --admin-server-ip "${admin_server_ip}"